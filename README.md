...
ツールでは実現できず(3パターン試行)いったん縮退し、Polar実レスポンス形式に忠実なモックで
全分岐(activate成功 / validate成功 / benefit_id不一致 / 404 / 403 / revoked / disabled /
オフライン)とUI反映を検証した。
- [x] **手動sandbox E2E(依頼者実施・2026-07-21)**: モック部分を実データ化して完了。
  テストカード購入→`?purchase=success`復帰→実キーでactivate→Pro解錠→リロード後validate維持→
  上限超過403(「Activation limit reached (3 devices)」)→DevTools Networkで
  「画像処理中POST 0件・ライセンス通信は小JSONのみ」まで全て実機確認。
  - 発見と対処: sandbox組織のbenefit IDは本番と別であり、初回activateが製品スコープ検証で
    正しく拒否された(設計どおりの挙動。`polar.config.local.js` にsandbox benefit IDを追記して解消)。
    拒否されたactivateもPolar側では枠を消費するため、上限テストは「合計4回目で拒否」として成立
  - 未実データ化の残り: **メールでのキー受領**(sandboxではPolarからのメールが届かず、キーは
    ダッシュボードから取得した)。メール経路は本番実購入E2E(下記)で確認する
