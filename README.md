...
## 素材ルール(全ツール共通)
サンプル/OG/SNS投稿用に使う画像は、次を**すべて**満たすものだけを使う:
- **人が写っていない・自前撮影・公共スペース**(施設内・私有地は不可)。
- **公開前にEXIF(特にGPS)を必ず除去**。
  - ⚠️ macOS プレビューの「書き出す」は GPS を消さない(実機の iPhone 写真で
    `exif:GPSLatitude`/`GPSLongitude` が Adobe XMP に残ったまま出力されることを確認済み)。
    **プレビューの書き出しを信用しない**。
  - 除去は `scripts/strip-exif.sh <file...>`(exiftool があれば `-all=`、無ければ Pillow で
    ピクセルのみ新規 Image に移して保存)を使うか、Canvas の `toBlob()` 経由で再生成する
    (副作用として全メタデータが落ちる。本サイトの LP サンプル生成パイプラインはこの方式)。
  - 除去後は `exiftool -G -a -s <file>` で EXIF/GPS/XMP/IPTC グループが無いことを確認する。
- **ファイル名・alt・キャプションに撮影地名を入れない**(汎用的な英語の内容説明のみ)。
- **撮影地は分散させる**(同一人物の居住地特定を避ける)。
- **画像のやり取りは、ファイル名だけでなく内容を言語化して併記する**
  (例:「Park_with_a_pine_grove.png = 松林と巨石のある公園、人物なし」)。
